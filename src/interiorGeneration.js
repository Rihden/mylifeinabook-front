import { drawCenteredParagraph, drawInteriorJustifiedParagraph, drawInteriorCenteredParagraph, drawStoryImage, addPage, drawOrnament } from './pdfUtils';
...
async function generateBook(book, bookStyles) {
    try {
        ...
        if (book.pagesCount % 2 == 1) {
            await pdfDoc.addPage([pageWidth * DISTANCE_PER_MM, pageHeight * DISTANCE_PER_MM])
        }
...
async function generateStory(content, options) {
    try {
        ...
        //Draw Ornament
        nextCoordinate = await drawOrnament(options.ornamentImage, { offsetY: nextCoordinate.offsetY, page: nextCoordinate.currentPage },)
